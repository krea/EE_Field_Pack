 ...
 */		
	...
	function index()
	{
		$this->EE->cp->set_variable('cp_page_title', lang('gallery_module_name'));
		return lang('gallery_cp_only_for_script');
	}	
...
//--------------------------------------------------------------------
	...
	function do_upload_file()
	{
		...
		if (is_array($_FILES))
		{
			...
			{
				if ($key == 'ft_gallery_file' AND @$_GET['dir'])
				{
		...
		if (isset($_GET['entry_id']) AND isset($_GET['dir']) AND isset($_GET['field_id']))
		{
			...
			if (isset($_SESSION['Ft_gallery']['cache'][$_GET['entry_id']][$_GET['field_id']]))
			{
				$cache = $_SESSION['Ft_gallery']['cache'][$_GET['entry_id']][$_GET['field_id']];
	...
	function _upload_file($key)
	{
		...
		$this->EE->load->library('filemanager');
		$data = $this->EE->filemanager->upload_file($key, 'ft_gallery_file');

		if (@$data["error"])
		{
			$data["name"] 	= $_FILES['ft_gallery_file']['name'];
			$data["size"] 	= filesize($_FILES['ft_gallery_file']['tmp_name']);
	...
	function _create_delete_url($dirname, $filename)
	{
		if (defined('BASE'))
		{
			return str_replace(AMP,'&',BASE).'&C=addons_modules&M=show_module_cp&module=gallery&method=do_upload_file&filename='.$filename.'&dir='.$dirname.'&hash='.md5($filename.$dirname.serialize($this->EE->config->config['site_url']).'del');
		}
		else
		{
