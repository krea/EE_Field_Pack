...
$('.ft_gallery_upload_form').each(
	function (k,v)
	{
		ft_upload_form_id 	= ($(v).attr("id"));
		ft_upload_config 	= ft_gallery_upload_form_config[ft_upload_form_id];
	    ...
	    {
	    	'maxNumberOfFiles' 	: parseInt(ft_upload_config.gallery_files_limit),
	    	'autoUpload'		: true,
			        	...
			        	for(var item in files) {	
			        		...
			        		for(var f in file) {
			        			var data = file[f];
		        				 ...
		        				 $('#' + ft_upload_form_id + ' .gallery_label_1:first').val(data.text_1);
		        				 $('#' + ft_upload_form_id + ' .gallery_label_1:first').removeClass('gallery_label_1');
		        				 
		        				 $('#' + ft_upload_form_id + ' .gallery_label_2:first').val(data.text_2);
		        				 $('#' + ft_upload_form_id + ' .gallery_label_2:first').removeClass('gallery_label_2');
		        				 
		        				 $('#' + ft_upload_form_id + ' .gallery_label_3:first').val(data.text_3);
		        				 $('#' + ft_upload_form_id + ' .gallery_label_3:first').removeClass('gallery_label_3');
		        				 
		        				 $('#' + ft_upload_form_id + ' .gallery_label_4:first').val(data.text_4);
		        				 $('#' + ft_upload_form_id + ' .gallery_label_4:first').removeClass('gallery_label_4');
		        				 
		        				 $('#' + ft_upload_form_id + ' .gallery_label_5:first').val(data.text_5);
		        				 $('#' + ft_upload_form_id + ' .gallery_label_5:first').removeClass('gallery_label_5');
			        		}
			        	}	                
